    ...
    class Manuall {
        ...
        public function addOrderHook($order_id) {
            ...
            if($success) {
                ...
                $note = '[POS FEEDBACK] Order could Successfully Pushed Manually '.$Order->id.". \r\n";
                $Order->add_order_note($note);
              
                $this->sendOrderSubmitSucessEmail( $Order->id, $note );  // send notification email
        ...
        function add_order_meta_box_actions( $actions ) {
            $actions['keeki_push_to_eastcoast'] = 'Push to Eastcoast1';
        ...
        private function sendOrderSubmitSucessEmail($order_id, $error_message)
        {
        	$to = defined('ORDER_ERROR_EMAIL_TO') ? ORDER_ERROR_EMAIL_TO : 'sunil.verma@webdesignmarket.com.au';
            $subject = defined('ORDER_ERROR_EMAIL_SUBJECT') ? ORDER_ERROR_EMAIL_SUBJECT : 'Keeki - Order Submit Failure';
            $url = KEEKI_API_URL;
            $message = "URL: $url\n";
            $message .= "Order # $order_id has sucessfully manually submited to Eastcoast. Please review the following error message and resubmit manually.\n\n";
            wp_mail( $to, $subject, $message );
        }
