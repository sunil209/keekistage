...
<?php 
		...
		$my_credit_pts  = get_user_meta( $get_user_id, 'user_credit_points' , true );
        $my_rewardspoints = get_user_meta( $get_user_id, 'user_rewards_points' , true ); 

?>
<p><?php printf( __( "You have %d %s", 'woocommerce-points-and-rewards' ), $points_balance, $points_label ); ?></p> 
<?php if($my_credit_pts){?> 
<p>You have <strong><?php echo '$'.$my_credit_pts; ?></strong> Credit Points And <strong><?php echo $my_rewardspoints; ?></strong> Rewards Points </p>
<?php } ?>
<?php if ( $events ) : ?>
