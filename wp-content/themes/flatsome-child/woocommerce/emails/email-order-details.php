			...
			if ( $totals ) {
				...
				foreach ( $totals as $total ) {
					...
					<tr>
					   ...
					   <?php
                            if ($total['label'] =='Cart Discount:' || $total['label']=='Order Discount:' || $total['label']=='Discount:') {
                                echo wp_kses_post( $total['label']).' <span style="color:#ff1010;">Promo Code: ';
                                $couponR = $order->get_used_coupons();
                                foreach ($couponR as $singleCoupon) {
                                    echo ucfirst($singleCoupon).'
                                    ';
                                }
                                echo '</span>';
                            } else {
